...
<div class="status"><div class="global-width">
	<%--<s:if test="${user==null}">请先登录</s:if>--%>
<span class="usertype">【登录角色：<s:property value="#session.user.sysPositionByPositionId.nameCn"/>】</span>
		<s:property value="#session.user.sysDepartmentByDepartmentId.name"/>	<s:property value="#session.user.name"/>你好！欢迎访问青鸟办公管理系统！<span style="margin-left:50px;">
<a href="loginOutSysEmployeeAction">退出</a></span></div></div>
<div class="main"><div class="global-width">
    ...
    <div class="action">
    	<div class="t">报销单列表</div>
   		<div class="pages">
        	<!--增加报销单 区域 开始-->
        	<form action="queryClaimAction.action" method="post">
                <table width="95%" border="0" cellspacing="0" cellpadding="0" class="list">
                	<tr>
                		<td colspan="6">
                			报销单状态
                			<s:select  name="claimParam.status" list="#{'':'全部','新创建':'新创建','已提交':'已提交','待付款':'待付款','已打回':'已打回','已拒绝':'已拒绝','已付款':'已付款','待审批':'待审批' }"></s:select>
                			开始时间
                			<s:textfield onfocus="WdatePicker({lang:'en'})" class="Wdate" id="startDate" name="claimParam.startDate"/>
                			结束时间
                			<s:textfield onfocus="WdatePicker({lang:'en'})" class="Wdate" id="endDate" name="claimParam.endDate"/>
                	...
                	<s:iterator value="claims">
                		<tr>
		                    ...
		                    <s:if test="#session.user.sysPositionByPositionId.id==1">
			                    <td>
			                      <img onclick="location.href='infoClaimAction.action?id=<s:property value="id"/>'" src="images/search.gif" width="16" height="15" />
			              		  <s:if test="status=='新创建' || status=='已打回'">
			                    	<img onclick="location.href='updateClaimAction.action?id=<s:property value="id"/>'" src="images/edit.gif" width="16" height="16" />
			                      	<img onclick="location.href='subClaimAction.action?id=<s:property value="id"/>'" src="images/sub.gif" width="16" height="16" />
			                      </s:if>
			                      <s:if test="status=='新创建'">
			                      	<img onclick="location.href='delClaimAction.action?id=<s:property value="id"/>'" src="images/delete.gif" width="16" height="16" />
			                      </s:if>
			                    </td>
		                    </s:if>
		                    <s:else>
			                    <td>
								  ...
								  <s:if test="#session.user.sysPositionByPositionId.nameCn=='部门经理' && status=='已提交'">
			                      	<img onclick="location.href='detailClaimAction.action?id=<s:property value="id"/>'" src="images/sub.gif" width="16" height="16" />
			                      </s:if>
                  ...
                  <tr>
                    <td colspan="6" align="center">
					 ...
					 <input type="hidden" name="page.pageAll" id="pageAll" value="<s:property value="page.pageAll"/>" />
